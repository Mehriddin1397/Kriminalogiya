...
class ResearchController extends Controller
{
    ...
    {
        $academia = Research::with(['photos'])->get();
        $categories = Category::forObjectType('research');
        return view('admin.research.index', compact('academia', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('file_path')) {
            $filePath = $request->file('file_path')->store('research/files', 'public');
        }

        $academia = Research::create([
        ...
        if ($request->has('categories')) {
            $academia->categories()->attach($request->categories, ['categorizable_type' => Research::class]);
        }
        return redirect()->route('research.index')->with('success', 'Academia muvaffaqiyatli yaratildi.');

    }

    public function update(Request $request, $id)
    {
        $academia = Research::findOrFail($id);
        $request->validate([
            'name_uz' => 'required|string|max:255',
            'name_ru' => 'required|string|max:255',
            'name_en' => 'required|string|max:255',
        ...
        return redirect()->route('research.index')->with('success', 'Academia muvaffaqiyatli yangilandi.');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('research.index')->with('success', 'Muvaffaqiyatli o‘chirildi.');
    }
}
