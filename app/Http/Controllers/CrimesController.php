...
class CrimesController extends Controller
{
    ...
    {
        $academia = Crimes::with(['photos'])->get();
        $categories = Category::forObjectType('crimes');
        return view('admin.crimes.index', compact('academia', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('categories')) {
            $academia->categories()->attach($request->categories, ['categorizable_type' => Crimes::class]);
        }
        return redirect()->route('crimes.index')->with('success', 'Academia muvaffaqiyatli yaratildi.');

    }

    public function update(Request $request, $id)
    {
        $academia = Crimes::findOrFail($id);
        $request->validate([
            'name_uz' => 'required|string|max:255',
            'name_ru' => 'required|string|max:255',
            'name_en' => 'required|string|max:255',
        ...
        return redirect()->route('crimes.index')->with('success', 'Academia muvaffaqiyatli yangilandi.');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('crimes.index')->with('success', 'Akademiya muvaffaqiyatli o‘chirildi.');
    }
}
