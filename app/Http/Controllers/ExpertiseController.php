...
class ExpertiseController extends Controller
{
    ...
    {
        $academia = Expertise::with(['photos'])->get();
        $categories = Category::forObjectType('expertise');
        return view('admin.expertise.index', compact('academia', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('categories')) {
            $academia->categories()->attach($request->categories, ['categorizable_type' => Expertise::class]);
        }
        return redirect()->route('expertise.index')->with('success', 'Academia muvaffaqiyatli yaratildi.');

    }

    public function update(Request $request, $id)
    {
        $academia = Expertise::findOrFail($id);
        $request->validate([
            'name_uz' => 'required|string|max:255',
            'name_ru' => 'required|string|max:255',
            'name_en' => 'required|string|max:255',
        ...
        return redirect()->route('expertise.index')->with('success', 'Academia muvaffaqiyatli yangilandi.');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('expertise.index')->with('success', 'Akademiya muvaffaqiyatli o‘chirildi.');
    }
}
