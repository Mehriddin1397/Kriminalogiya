...
class ArticlesController extends Controller
{
    ...
    {
        $academia = Articles::with(['photos'])->get();
        $categories = Category::forObjectType('articles');
        return view('admin.articles.index', compact('academia', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('file_path')) {
            $filePath = $request->file('file_path')->store('articles/files', 'public');
        }

        $academia = Articles::create([
        ...
        if ($request->has('categories')) {
            $academia->categories()->attach($request->categories, ['categorizable_type' => Articles::class]);
        }
        return redirect()->route('articles.index')->with('success', 'Academia muvaffaqiyatli yaratildi.');

    }

    public function update(Request $request, $id)
    {
        $academia = Articles::findOrFail($id);
        $request->validate([
            'name_uz' => 'required|string|max:255',
            'name_ru' => 'required|string|max:255',
            'name_en' => 'required|string|max:255',
        ...
        return redirect()->route('articles.index')->with('success', 'Academia muvaffaqiyatli yangilandi.');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('articles.index')->with('success', 'Akademiya muvaffaqiyatli o‘chirildi.');
    }
}
