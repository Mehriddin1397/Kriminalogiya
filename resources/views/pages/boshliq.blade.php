    ...
    <section class="lx-section" style="background: var(--lx-cream);">
        <div class="container">
            ...
            @if($boss->count())
                @php
                    $chief = $boss->first();
                    $deputies = $boss->slice(1);
                @endphp

                {{-- Institut rahbari (boshliq) --}}
                @php
                    $chiefPhoto = $chief->photos->first();
                    $chiefModalId = 'bossModal' . $chief->id;
                    $chiefInitial = mb_strtoupper(mb_substr(trim($chief->name ?? '?'), 0, 1));
                @endphp

                <a href="#"
                   role="button"
                   class="lx-leader-chief"
                   data-bs-toggle="modal"
                   data-bs-target="#{{ $chiefModalId }}"
                   data-aos="fade-up">
                    ...
                    <div class="lx-leader-chief-body">
                        <span class="lx-leader-chief-badge">{{ __('lan.ins_rahbari') }}</span>
                        <h3 class="lx-leader-chief-name">{{ $chief->name }}</h3>
                        <div class="lx-leader-chief-post">{{ $chief->post }}</div>
                ...
                <div class="modal fade lx-modal"
                     id="{{ $chiefModalId }}"
                     tabindex="-1"
                     aria-labelledby="{{ $chiefModalId }}Label"
                     aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                                <div>
                                    <h5 class="modal-title" id="{{ $chiefModalId }}Label">{{ $chief->name }}</h5>
                                    <div class="lx-leader-eyebrow" style="margin-top:6px;">{{ $chief->post }}</div>
                                </div>
                            ...
                            <div class="modal-body">
                                ...
                                @if(!empty($chief->email))
                                    <div class="lx-modal-row">
                                        ...
                                        <span class="lx-modal-value">
                                            <a href="mailto:{{ $chief->email }}">{{ $chief->email }}</a>
                                        </span>
                                    </div>
                                @endif

                                @if(!empty($chief->worktime))
                                    <div class="lx-modal-row">
                                        <span class="lx-modal-label">{{ __('lan.ish_jadvali') }}</span>
                                        <span class="lx-modal-value">{{ $chief->worktime }}</span>
                                    </div>
                                @endif
                            </div>
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <div>
                                                ...
                                                <div class="lx-leader-eyebrow" style="margin-top:6px;">{{ $bos->post }}</div>
                                            </div>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ __('lan.yopish') }}">
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                                                    <path d="M18 6 6 18M6 6l12 12"/>
